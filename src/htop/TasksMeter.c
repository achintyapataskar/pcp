...
static void TasksMeter_setValues(Meter* this, char* buffer, int len) {
   ProcessList* pl = this->pl;
	...
	int sts;
	char *pmclient_init[] = {"proc.runq.kernel", "kernel.all.runnalble", "proc.nprocs"};
	pmID pmidlist[3];
	pmDesc desclist[3];
	pmResult *rp;
	pmAtomValue atom;
	sts = pmLookupName(3, pmclient_init, pmidlist);
	sts = pmLookupDesc(pmidlist[0], desclist);
	sts = pmLookupDesc(pmidlist[1], desclist);
	sts = pmLookupDesc(pmidlist[2], desclist);
	sts = pmFetch(3, pmidlist, &rp);
	pmExtractValue(rp->vset[0]->valfmt, rp->vset[0]->vlist, desclist[0].type, &atom, PM_TYPE_U32);
	this->values[0] = atom.ul;
	pmExtractValue(rp->vset[1]->valfmt, rp->vset[1]->vlist, desclist[1].type, &atom, PM_TYPE_U32);
	this->values[1] = atom.ul;
	pmExtractValue(rp->vset[2]->valfmt, rp->vset[2]->vlist, desclist[2].type, &atom, PM_TYPE_U32);
	this->values[3] = atom.ul;
	pmFreeResult(rp);
